        ...
        if len(KBar.GetOpen()) > 31:
            ...
            if CPrice >= Ceil + Spread:
                ...
                if OrderInfo == False:
                    ...
                    print(Prod,'買進時間',OrderInfoTime,'買進價格',OrderInfoPrice,'停損價位',StopLossPoint,'停利價位',TakeProfitPoint)
                    continue
                ...
                if OrderInfo == False:
                    ...
                    # 紀錄固定停損停利價位
                    StopLossPoint= OrderInfoPrice *(1+StopLoss)
                    print(Prod,'賣出時間',OrderInfoTime,'賣出價格',OrderInfoPrice,'停損價位',StopLossPoint,'停利價位',TakeProfitPoint)
                    continue
    # 判斷時間是否到達出場時間
        ...
        if CTime > CoverTime:
            ...
            if OrderInfo == False:
                ...
                GO.EndSubscribe()
        # 當價格向上移動 則停損點數也向上移動
        elif CPrice *(1-StopLoss) > StopLossPoint:
            StopLossPoint = CPrice *(1-StopLoss)
            continue
        # 到達移動停損價位則出場
