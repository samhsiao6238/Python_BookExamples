...
for row in GO.Subscribe(Broker, 'match', Prod):
    ...
    if KBar.AddPrice(Time,Price,Qty) == 1:
        ...
        if len(CloseList) >= LongRSIPeriod+2:
            LongRSIList = KBar.GetRSI(LongRSIPeriod)
            ShortRSIList=KBar.GetRSI(ShortRSIPeriod)
            ClosePrice=CloseList[-2]
            LongRSI=LongRSIList[-2]
            ShortRSI=ShortRSIList[-2]
            LastClosePrice=CloseList[-3]
            LastLongRSI=LongRSIList[-3]
            LastShortRSI=ShortRSIList[-3]
            print(RC.GetOpenInterest(),Time,'最新收盤價',ClosePrice,'上一筆收盤價',LastClosePrice,'最新短RSI',ShortRSI,'上一筆最新短RSI',LastShortRSI,'最新長RSI',LongRSI,'上一筆最新長RSI',LastLongRSI)
            # 判斷進場的部分
            if RC.GetOpenInterest() == 0:
                # 黃金交叉 買進多單
                if LastShortRSI <= LastLongRSI and ShortRSI > LongRSI:
                    ...
                    if OrderInfo == False:
                        ...
                        GO.EndSubscribe()
                # 死亡交叉 買進空單
                elif LastShortRSI >= LastLongRSI and ShortRSI < LongRSI:
