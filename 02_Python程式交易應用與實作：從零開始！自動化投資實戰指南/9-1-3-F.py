...
CoverTime = datetime.datetime.strptime(Today + ' 20:25:00','%Y%m%d %H:%M:%S')
        ...
        if len(KBar.GetOpen()) > 31:
            ...
            if CPrice >= Ceil + Spread:
                ...
                if OrderInfo == False:
                    ...
                    TakeProfitPoint= OrderInfoPrice +TakeProfit
                ...
                if OrderInfo == False:
                    ...
                    StopLossPoint= OrderInfoPrice +StopLoss
                    TakeProfitPoint= OrderInfoPrice -TakeProfit
                    print(Prod,'空單買進時間',OrderInfoTime,'買進價格',OrderInfoPrice,'停損價位',StopLossPoint,'停利價位',TakeProfitPoint)
                    continue
    # 判斷時間是否到達出場時間
        ...
        # 到達停利價位則出場
            ...
            if OrderInfo == False:
                ...
                OrderInfoPrice=float(OrderInfo[4])            
                RC.Cover('S',Prod,OrderInfoTime,OrderInfoPrice,Qty)
                print(Prod,'多單停利平倉時間',OrderInfoTime,'平倉價格',OrderInfoPrice)
                GO.EndSubscribe()
    # 判斷時間是否到達出場時間
    elif RC.GetOpenInterest() == -1:
        # 超過出場時間則出場
        ...
        # 到達停利價位則出場
            ...
            if OrderInfo == False:
                ...
                OrderInfoPrice=float(OrderInfo[4])            
                RC.Cover('B',Prod,OrderInfoTime,OrderInfoPrice,Qty)
                print(Prod,'空單停利平倉時間',OrderInfoTime,'平倉價格',OrderInfoPrice)
                GO.EndSubscribe()
